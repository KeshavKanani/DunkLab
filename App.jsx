...
html,body{background:#07070D;color:#F0F0F0;font-family:'Barlow Condensed',sans-serif;-webkit-font-smoothing:antialiased;overflow-x:hidden}
::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:#1C1C2A;border-radius:2px}
input[type=number],input[type=text],select{-webkit-appearance:none;-moz-appearance:textfield;font-family:'DM Mono',monospace;background:#0C0C14;border:1px solid #1C1C2A;color:#F0F0F0;border-radius:6px;outline:none;width:100%}
input::-webkit-outer-spin-button,input::-webkit-inner-spin-button{-webkit-appearance:none}
button{cursor:pointer;-webkit-tap-highlight-color:transparent}
.fade{animation:fu .24s cubic-bezier(.16,1,.3,1) both}
.pop{animation:pp .32s cubic-bezier(.16,1,.3,1) both}
.bounceIn{animation:bi .45s cubic-bezier(.34,1.56,.64,1) both}
.slideup{animation:su .3s cubic-bezier(.16,1,.3,1) both}
@keyframes fu{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
@keyframes pp{from{opacity:0;transform:scale(.93)}to{opacity:1;transform:scale(1)}}
@keyframes bi{from{opacity:0;transform:scale(.55)}to{opacity:1;transform:scale(1)}}
@keyframes su{from{transform:translateY(100%)}to{transform:translateY(0)}}
@keyframes glow{0%,100%{box-shadow:0 0 0 0 #FF4D0000}50%{box-shadow:0 0 24px 3px #FF4D0055}}
@keyframes xpUp{0%{opacity:1;transform:translateY(0)}100%{opacity:0;transform:translateY(-54px)}}
@keyframes coachPulse{0%,100%{border-color:#FF4D0033}50%{border-color:#FF4D0088}}
@keyframes typingDot{0%,80%,100%{transform:scale(0);opacity:.3}40%{transform:scale(1);opacity:1}}
@keyframes coachGlow{0%,100%{box-shadow:0 0 0 0 #FF4D0000}50%{box-shadow:0 0 20px 2px #FF4D0030}}
@keyframes doneGlow{0%{box-shadow:0 0 0 0 #38B56A00}40%{box-shadow:0 0 12px 3px #38B56A55}100%{box-shadow:0 0 0 0 #38B56A00}}
...
.glowbtn{animation:glow 2s ease infinite}
.coachpulse{animation:coachPulse 2s ease infinite}
.coachglow{animation:coachGlow 3s ease infinite}
.doneglow{animation:doneGlow .6s ease}
.streakpulse{animation:streakPulse .4s cubic-bezier(.34,1.56,.64,1)}
.press:active{transform:scale(.96);transition:transform .1s}
.rankcard{transition:border-color .2s,box-shadow .2s,transform .15s}
.rankcard:hover{transform:translateY(-1px)}
.badgecard{transition:border-color .2s,box-shadow .2s,transform .15s}
.badgecard:hover{transform:scale(1.03)}
.td1{display:inline-block;width:6px;height:6px;border-radius:50%;background:#FF4D00;animation:typingDot 1.2s ease infinite;margin:0 2px}
.td2{display:inline-block;width:6px;height:6px;border-radius:50%;background:#FF4D00;animation:typingDot 1.2s ease .2s infinite;margin:0 2px}
.td3{display:inline-block;width:6px;height:6px;border-radius:50%;background:#FF4D00;animation:typingDot 1.2s ease .4s infinite;margin:0 2px}
.xpfloat{position:fixed;pointer-events:none;font-family:'Bebas Neue',sans-serif;font-size:22px;color:#FFB800;animation:xpUp 1s ease both;z-index:9999;text-shadow:0 0 18px #FFB80099}
`;
...
const LEVELS = [
  {id:1,label:"Can't Touch Net",  vert:17,icon:"🪑",color:"#555"},
  {id:2,label:"Touches Net",      vert:19,icon:"🤏",color:"#777"},
  {id:3,label:"Touches Backboard",vert:22,icon:"📋",color:"#999"},
  {id:4,label:"Touches Rim",      vert:24,icon:"🤙",color:"#C8A55A"},
  {id:5,label:"Grabs Rim",        vert:26,icon:"✊",color:"#D4A017"},
  {id:6,label:"Hangs on Rim",     vert:28,icon:"⭐",color:"#FFB800"},
  {id:7,label:"Almost Dunking",   vert:30,icon:"🔥",color:"#FF4D00"},
  {id:8,label:"CAN DUNK",         vert:32,icon:"👑",color:"#FF0040"},
];
...
function Milestone({msg,sub,onClose}) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
      <div className="bounceIn" style={{background:"linear-gradient(135deg,#0D0D16,#150A06)",border:`2px solid ${C.orange}`,borderRadius:12,padding:"30px 26px",textAlign:"center",maxWidth:300,width:"100%"}}>
        <div style={{fontSize:52,marginBottom:10}}>🎉</div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:C.orange,letterSpacing:".05em",lineHeight:1.1,marginBottom:8}}>{msg}</div>
        {sub&&<div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:C.muted,marginBottom:18,lineHeight:1.6}}>{sub}</div>}
        <button onClick={onClose} style={{background:C.orange,color:"#000",border:"none",fontFamily:"'Bebas Neue',sans-serif",fontSize:15,letterSpacing:".1em",padding:"11px 26px",borderRadius:5}}>LET'S GO</button>
      </div>
    </div>
  );
}
...
function ProModal({onClose,onUpgrade,gap,wkSess}) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.93)",zIndex:999,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div className="slideup" style={{background:"#0D0D16",border:`1px solid ${C.orange}`,borderRadius:"14px 14px 0 0",width:"100%",maxWidth:520,padding:"24px 18px 38px",maxHeight:"92vh",overflowY:"auto",position:"relative"}} onClick={e=>e.stopPropagation()}>
        <button onClick={onClose} style={{position:"absolute",top:14,right:16,background:"none",border:"none",color:"#555",fontSize:22,lineHeight:1}}>✕</button>

        {/* Hook line */}
        <div style={{background:`${C.orange}12`,border:`1px solid ${C.orange}33`,borderRadius:7,padding:"10px 13px",marginBottom:14}}>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:15,color:"#F0F0F0",lineHeight:1.4}}>You're already training. The difference is doing it right.</div>
        </div>

        <div style={{fontFamily:"'DM Mono',monospace",fontSize:8,color:C.orange,letterSpacing:".16em",marginBottom:5}}>THE DIFFERENCE</div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,letterSpacing:".03em",lineHeight:1.1,marginBottom:5}}>MOST PLAYERS NEVER DUNK<br/>BECAUSE THEY TRAIN RANDOMLY.</div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:C.orange,letterSpacing:".04em",marginBottom:14}}>THIS FIXES THAT.</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
          {[
            ...
            <div key={si} style={{background:si===0?"#0A0A12":"#0A120A",border:`1px solid ${side.col}30`,borderRadius:7,padding:"11px 12px"}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:8,color:side.col,letterSpacing:".1em",marginBottom:7}}>{side.title}</div>
              {side.items.map(t=>(
                <div key={t} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:5}}>
                  ...
                  <span style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:si===0?C.muted:"#C8C8C8",lineHeight:1.4}}>{t}</span>
                </div>
              ))}
            </div>
          ))}
...
function Onboarding({calcRes,onComplete}) {
  ...
  const [d,setD]=useState({name:"",age:"",height:calcRes?.h?.toString()||"",level:calcRes?((LEVELS.slice().reverse().find(l=>calcRes.v>=l.vert)||LEVELS[0]).id):3,skill:"beginner"});
...
export default function App() {
  ...
  // Derived data
  const D = {
    name:raw.name||"ATHLETE", height:raw.height||"", level:raw.level||3,
    ...
    <div className="fade">
      ...
      {coachOpen&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:998,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>{setCoachOpen(false);setCoachQ(null);setCoachTextReply(null);setCoachTyping(false);}}>
          <div className="slideup coachglow" style={{background:"#0D0D15",border:`1px solid ${C.orange}55`,borderRadius:"14px 14px 0 0",width:"100%",maxWidth:520,maxHeight:"90vh",overflowY:"auto",position:"relative"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 16px 0"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,letterSpacing:".06em",lineHeight:1}}>COACH</div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"#9090AA",marginTop:1}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <button onClick={()=>{setCoachOpen(false);setCoachQ(null);setCoachTextReply(null);setCoachTyping(false);}} style={{background:"#1A1A24",border:`1px solid ${C.border}`,color:"#888",borderRadius:6,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>✕</button>
              </div>
            </div>
    ...
    <div className="fade">
      ...
      {allBadgesUnlocked ? (
        <div className="pop" style={{background:`linear-gradient(135deg,${C.gold}18,${C.orange}12)`,border:`2px solid ${C.gold}`,borderRadius:8,padding:"13px 14px",marginBottom:10,textAlign:"center"}}>
          ...
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:C.muted}}>You've hit every milestone. Elite status achieved.</div>
        </div>
      ) : (
                  ...
                  return (
                    <div key={i} style={{display:"flex",alignItems:"center",flex:1}}>
                      <button onClick={()=>{if(!prevPhaseDone){showT(`🔒 Finish ${PRO_WEEKS[i-1].phase} first`);return;}setProWeek(i);}}
                        style={{flex:1,background:isActive?`${w.color}20`:prevPhaseDone?"#111":"#0A0A0E",border:`1px solid ${isActive?w.color:prevPhaseDone?C.border:"#1A1A22"}`,color:isActive?w.color:prevPhaseDone?"#555":"#2A2A38",borderRadius:5,padding:"8px 4px",textAlign:"center",transition:"all .15s",position:"relative"}}>
                  ...
                  return (
                    <div key={i} style={{display:"flex",alignItems:"center",flex:1}}>
                      <button onClick={()=>{
                        ...
                        setGymPhase(i);
                      }} style={{flex:1,background:isActive?`${C.purple}22`:prevGymDone?"#111":"#0A0A0E",border:`1px solid ${isActive?C.purple:prevGymDone?C.border:"#1A1A22"}`,color:isActive?C.purple:prevGymDone?"#555":"#2A2A38",borderRadius:5,padding:"7px 4px",textAlign:"center",transition:"all .15s",position:"relative",fontSize:1}}>
